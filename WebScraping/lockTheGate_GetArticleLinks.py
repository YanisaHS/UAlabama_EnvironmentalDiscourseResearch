...
def randomSleepsFunction():
    return random.randrange(10, 60, 1) / 10.0 #0.1 is step size - so it can randomize 1.1, 1.2, 1.3, ... 3.9, 4.0 etc.
...
url = 'https://www.lockthegate.org.au/media_releases'
with webDriver as driver:
    ...
    while True:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "content-wrap")))
                break
        ...
        allURLs = soup.find_all('div', attrs={'class': 'content-wrap'})
        try:
            for eachURL in allURLs:
                inProcessURL = eachURL.find('a')
                url = inProcessURL['href']
                finalListOfArticleURLs.append(url)
        except Exception as exception:
            print('URL skipped - caused exception: ' + eachURL)
            print(exception)
    
        # Click next page button if it exists
        try:
            olderPostsButton = driver.find_element_by_class_name('pagination')
            inProcessButton = olderPostsButton.find_elements_by_tag_name('li')
            isThereNextPage = inProcessButton[-1].get_attribute('class')
            if isThereNextPage != 'disabled':
                print('Opening page #' + str(pageNumber))
                pageNumber = pageNumber + 1
                inProcessButton[-1].click()
            else:
                break
        except:
            break
...
fileToWrite = open('lockTheGate_ArticleLinks.txt', 'w')
for eachLink in finalListOfArticleURLs:
    # write each link to a new line
    fileToWrite.write('https://www.lockthegate.org.au')
    fileToWrite.write(eachLink)
    fileToWrite.write('\n')
fileToWrite.close()