...
pageNumber = 1 # to get it started - will increment below
url = 'https://www.evergladesfoundation.org/blog/page/{}/'.format(pageNumber)
with webDriver as driver:
    ...
    while True:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "post-list-item-wrapper")))
                break
        ...
        allURLs = soup.find_all('div', attrs={'class': 'blog-post-homepage-TOI-description-fill'})
        try:
        ...
        # Go on to the next page by changing the url page number
        pageNumber = pageNumber + 1
        if pageNumber == 25:
            break
        else:
            url = 'https://www.evergladesfoundation.org/blog/page/{}/'.format(pageNumber)
            print('Getting page number ' + str(pageNumber))
            driver.get(url)

# Write list of links to a file:
fileToWrite = open('evergladesFoundation_ArticleLinks.txt', 'w')
