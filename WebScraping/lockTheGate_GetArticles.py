...
from Classes.lockTheGateArticleInformation import LockTheGateArticleInformation
...
links = open('lockTheGate_ArticleLinks.txt').read()
...
with webDriver as driver:
    ...
    for eachURL in listOfLinks:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "headline")))
                break
        ...
        try:
            # Headline
            inProcessHeadline = soup.find('div', attrs={'class': 'headline'})
            headline = inProcessHeadline.text

            # Organization
            organization = 'Lock The Gate'

            # Date
            inProcessDate = soup.find('div', attrs={'id': 'content'})
            ...
            # Text
            text = ''
            inProcessText = soup.find('div', attrs={'class': 'intro'})
            ...
            putThingsInClass = LockTheGateArticleInformation(headline, organization, date, url, text)
...
beginningFilePath = '/Users/yanisa/GoogleDrive/Data_Research/RA_Fall2020_UAlabama/scrapedArticles/lockTheGate/AllArticles/'
