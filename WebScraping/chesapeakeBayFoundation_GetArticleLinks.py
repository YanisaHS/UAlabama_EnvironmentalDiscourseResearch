...
url = 'https://www.cbf.org/blogs/save-the-bay/'
with webDriver as driver:
    ...
    while True:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "title-heading")))
                break
        ...
        allURLs = soup.find_all('h2', attrs={'class': 'title-heading'})
        try:
        ...
        try:
            olderPostsButton = soup.find('td', attrs={'class': 'pg-next'})
            ...
            driver.get('https://www.cbf.org/' + olderPostsButtonPageURL)
        except:
            break

# Write list of links to a file:
fileToWrite = open('chesapeakeBayFoundation_ArticleLinks.txt', 'w')
