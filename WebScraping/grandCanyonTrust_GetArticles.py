...
from Classes.grandCanyonTrustArticleInformation import GrandCanyonTrustArticleInformation
...
links = open('grandCanyonTrust_ArticleLinks.txt').read()
...
with webDriver as driver:
    ...
    for eachURL in listOfLinks:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "blog")))
                break
        ...
        try:
            # Headline
            inProcessHeadlineFirst = soup.find('div', attrs={'class': 'blog'})
            ...
            # Date
            inProcessDate = soup.find('div', attrs={'class': 'bylineDate'})
            ...
            # Text
            text = ''
            inProcessText = soup.find('div', attrs={'id': 'blogContent'})
            ...
            for paragraph in inProcessTextSecond[1:]:
                text = text + ' ' + paragraph.text
                
            # Put things in class, save info, increment article count
            putThingsInClass = GrandCanyonTrustArticleInformation(headline, organization, date, url, text)
...
beginningFilePath = '/Users/yanisa/GoogleDrive/Data_Research/RA_Fall2020_UAlabama/scrapedArticles/grandCanyonTrust/'
