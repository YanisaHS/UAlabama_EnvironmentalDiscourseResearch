...
from Classes.evergladesFoundationArticleInformation import EvergladesFoundationArticleInformation
...
links = open('evergladesFoundation_ArticleLinks.txt').read()
...
with webDriver as driver:
    ...
    for eachURL in listOfLinks:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "blog-post-page-font")))
                break
        ...
        try:
            # Headline
            inProcessHeadlineFirst = soup.find('span', attrs={'class': 'blog-post-title-font'})
            headline = inProcessHeadlineFirst.text

            # Organization
            organization = 'Everglades Foundation'

            # Date
            inProcessDate = soup.find('span', attrs={'class': 'time-ago'})
            ...
            # Text
            text = ''
            inProcessText = soup.find('div', attrs={'class': 'blog-post-page-font'})
            ...
            putThingsInClass = EvergladesFoundationArticleInformation(headline, organization, date, url, text)
...
beginningFilePath = '/Users/yanisa/GoogleDrive/Data_Research/RA_Fall2020_UAlabama/scrapedArticles/evergladesFoundation/'
