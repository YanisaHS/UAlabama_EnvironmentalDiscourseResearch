...
from Classes.chesapeakeBayFoundationArticleInformation import ChesapeakeBayFoundationArticleInformation
...
links = open('chesapeakeBayFoundation_ArticleLinks.txt').read()
...
with webDriver as driver:
    ...
    for eachURL in listOfLinks:
        ...
        while retries < 2:
            try:
                wait.until(presence_of_element_located((By.CLASS_NAME, "body-text")))
                break
        ...
        try:
            # Headline
            inProcessHeadlineFirst = soup.find('div', attrs={'class': 'col-12 page'})
            inProcessHeadlineSecond = soup.find('h1')
            headline = inProcessHeadlineSecond.text

            # Organization
            organization = 'Chesapeake Bay Foundation'

            # Date
            inProcessDate = soup.find('span', attrs={'class': 'date'})
            if inProcessDate == None:
                date = '01 XXX 1111 - FIX NEEDED - GET DATE'
            else:
            ...
            # Text
            text = ''
            inProcessText = soup.find('div', attrs={'class': 'body-text'})
            # recursive=False so it doesn't go into the child class that isn't p to get the p classes inside that not-p child class
            ...
            putThingsInClass = ChesapeakeBayFoundationArticleInformation(headline, organization, date, url, text)
...
beginningFilePath = '/Users/yanisa/GoogleDrive/Data_Research/RA_Fall2020_UAlabama/scrapedArticles/chesapeakeBayFoundation/'
